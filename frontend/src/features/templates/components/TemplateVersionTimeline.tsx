...
interface TemplateVersionTimelineProps {
  ...
  onRestore: (version: PromptTemplateVersion, changelog?: string) => void;
  isRestoring?: boolean;
}

export const TemplateVersionTimeline = ({
  versions,
  onRestore,
  isRestoring
}: TemplateVersionTimelineProps) => {
  const [selected, setSelected] = useState<PromptTemplateVersion | null>(null);
            ...
            <div>
              ...
              <p>
                ...
                {selected.placeholders.length > 0
                  ? selected.placeholders.join(", ")
                  : "n/a"}
              </p>
              <p>
                <strong>Render example:</strong>{" "}
                {selected.render_example || "n/a"}
              </p>
              <label>
