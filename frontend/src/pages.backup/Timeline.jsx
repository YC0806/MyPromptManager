...
function TimelineEvent({ event, isLast }) {
  ...
  return (
    <div className="relative pl-8">
      {/* Timeline Line */}
      {!isLast && (
        <div className="absolute left-4 top-12 bottom-0 w-px bg-zinc-200 dark:bg-zinc-700"></div>
      )}

      {/* Timeline Dot */}
      <div className={`absolute left-0 top-4 w-8 h-8 rounded-full flex items-center justify-center ${
        isRelease ? 'bg-teal-100 dark:bg-teal-900/30' : 'bg-yellow-100 dark:bg-yellow-900/30'
      }`}>
        {isRelease ? (
